...
p_0 = 0.1;     % Initial roll rate (rad/s)
q_0 = 0.08;     % Initial pitch rate (rad/s)
r_0 = 0.05;     % Initial yaw rate (rad/s)
...
t_span = [0 30]; % Simulation time span (start time, end time)

% Define initial conditions vector
initial_conditions = [phi_0; theta_0; psi_0; p_0; q_0; r_0];

euler_dynamics = @(t, x) [...
    x(4) + x(5) * sin(x(1)) * tan(x(2)) + x(6) * cos(x(1)) * tan(x(2));...
    x(5) * cos(x(1)) - x(6) * sin(x(1));...
    x(5) * sin(x(1)) * sec(x(2)) + x(6) * cos(x(1)) * sec(x(2));...
    -0.01 * x(4);... % Damping torque (thrusters firing to reduce roll rate)
    -0.01 * x(5);... % Damping torque (thrusters firing to reduce pitch rate)
    -0.01 * x(6)];   % Damping torque (thrusters firing to reduce yaw rate)
    0];
